...
public class CuckooHashTable<AnyType>
{
     ...
     */
    public CuckooHashTable( HashFamily<? super AnyType> hf, @NonNegative int size )
    {
    ...
    private boolean insertHelper1( AnyType x )
    {
        ...
        while( true )
        {
            @IndexOrLow("array") int lastPos = -1;
            @IndexFor("array") int pos;
    ...
    private boolean insertHelper2( AnyType x )
    {
        ...
        while( true )
        {
            for( int count = 0; count < COUNT_LIMIT; count++ )
            {
                @IndexFor("array") int pos = myhash( x, count % numHashFunctions );
    ...
    private @IndexFor("array") int myhash( AnyType x, @NonNegative int which )
    {
    ...
    {
        @IndexOrLow("array") int pos = findPos( x );
    ...
    @NonNegative private int currentSize;              // The number of occupied cells
     ...
     */
    protected static @NonNegative int nextPrime(@NonNegative int n )
    {
        if( n % 2 == 0 )
            n++;
     ...
     */
    private static boolean isPrime(@NonNegative int n )
    {
        if( n == 2 || n == 3 )
            return true;
