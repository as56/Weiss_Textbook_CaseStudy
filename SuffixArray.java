...
class SuffixArray
{
    ...
    {
        @IndexOrHigh("sa") int N = sa.length;
        int @SameLen("sa") [ ] rank = new int[ N ];
     ...
     */
    public static void createSuffixArray( String str, int @SameLen("#1")[ ] sa, int @SameLen("#1") [ ] LCP )
    {        
        int N = str.length( );        
        int [ ] s = new int[ N + 3 ];
        int @SameLen("s")[ ] SA = new int[ N + 3 ];
    ...
    public static void makeSuffixArray( int @MinLen(5)[ ] s, int [ ] SA, @IndexFor("#1") int n, int K )
    {
        int n0 = ( n + 2 ) / 3;
        int n1 = ( n + 1 ) / 3;
        int n2 = n / 3;
        ...
        int @SameLen("s12") [ ] SA12 = new int[ n12 + 3 ];
        int [ ] s0   = new int[ n0 ];
        int @SameLen("s0") [ ] SA0  = new int[ n0 ];
    ...
    private static boolean suffix12IsSmaller( int [ ] s, int [ ] s12, int [ ] SA12,
                                             int n0, int i, int j, @IndexFor("#3") int t )
    {
    ...
    public static boolean isPermutation( int [ ] SA, @IndexFor("#1") int n )
    {
    ...
    public static boolean sleq( int  [ ] s1, @IndexFor("#1") int start1, int [ ] s2, @IndexFor("#3") int start2 )
    {
    ...
    public static boolean isSorted( int [ ] SA, int [ ] s, @Positive @LTLengthOf("#1") int n )
    {
        for( int i = 0; i < n-1; i++ )
    ...
    {        
        ...
        int @SameLen("suffarr") [ ] LCP = new int[ str.length( ) ];

        createSuffixArray( str, sufarr, LCP );

        @IndexOrHigh("sufarr") int N = str.length();        
     ...
     */
    public static void createSuffixArraySlow( String str, int @MinLen(1) @SameLen("#1")[ ] SA, int @MinLen(1) @SameLen("#1")[ ] LCP )
    {
        if( SA.length != str.length( ) || LCP.length != str.length( ) )
            throw new IllegalArgumentException( );
        
        
        String @SameLen("str") [ ] suffixes = new String[ str.length() ];
        
        @IndexOrHigh("suffixes") int N = str.length( );
